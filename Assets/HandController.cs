...
public class HandController: MonoBehaviour
{
    ...
    gDefine.Direction HandDirection(Vector2 StartPos, Vector2 EndPos)
    {
        ...
        //手指水平移動
        if (Mathf.Abs(StartPos.x - EndPos.x) > Mathf.Abs(StartPos.y - EndPos.y) && Mathf.Abs(StartPos.x - EndPos.x) > 100)
        {
            if (StartPos.x > EndPos.x && Mathf.Abs(StartPos.x - EndPos.x)>100)
            {
                //手指向左滑動
                //減rotation y
                mDirection = gDefine.Direction.Left;
                //unitychan.transform.rotation *= new Quaternion(0, 0.707f, 0, 0.707f);
                charactertwo.GetComponent<Rigidbody>().position = new Vector3(10, -2, -5);
                unitychan.GetComponent<Rigidbody>().position = new Vector3(0, -2, -5);
                //換model
                print("OMG");
            }
            else
            {
                ...
                unitychan.GetComponent<Rigidbody>().position = new Vector3(10,-2,-5);
                charactertwo.GetComponent<Rigidbody>().position = new Vector3(0, -2, -5);
                //換model
                print("fuck");
            }
        }
